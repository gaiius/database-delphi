...
rs.Open "SELECT * FROM tb_divisi WHERE kd_divisi='" & Trim(txt_kode.Text) & "'", cn, 1, 2
If rs.EOF Then
...
Else
    If MsgBox("Anda yakin akan menghapus data : " & txt_nama.Text, vbYesNo, "Hapus Data") = vbYes Then
        Set rs = New ADODB.Recordset
        rs.Open "DELETE FROM tb_divisi WHERE kd_divisi='" & Trim(txt_kode.Text) & "'", cn, 1, 2
        Kosong
        TampilData
    End If
End If
...
If rs.EOF Then
    rs.AddNew
    rs!kd_divisi = Trim(txt_kode.Text)
    rs!divisi = Trim(txt_nama.Text)
    rs.Update
Else
    rs!divisi = Trim(txt_nama.Text)
    rs.Update
...
On Error Resume Next
  Dim rs As New ADODB.Recordset
    ...
    Set rs = New ADODB.Recordset
    rs.Open "SELECT * FROM tb_divisi WHERE kd_divisi='" & pID & "'", cn, 1, 2
    If rs.EOF Then
        GetList = False
    Else
    With rs
        txt_kode.Text = !kd_divisi
        txt_nama.Text = !divisi
...
Sub TampilData()
Set rs = New ADODB.Recordset
rs.Open "SELECT * FROM tb_divisi", cn, 1, 2
