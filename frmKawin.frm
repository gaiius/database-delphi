...
Begin VB.Form frmKawin 
   ...
   Begin Threed.SSPanel Panel 
      ...
      Begin Threed.SSPanel SSPanel1 
         ...
         _ExtentY        =   873
         _StockProps     =   15
         Caption         =   "INPUT STATUS PERKAWINAN "
         ForeColor       =   12632064
         BackColor       =   16576
         BeginProperty Font {0BE35203-8F91-11CE-9DE3-00AA004BB851} 
            Name            =   "Tahoma"
...
rs.Open "SELECT * FROM tb_status_kawin WHERE kd_kawin='" & Trim(txt_kode.Text) & "'", cn, 1, 2
If rs.EOF Then
...
Else
    If MsgBox("Anda yakin akan menghapus data : " & txt_nama.Text, vbYesNo, "Hapus Data") = vbYes Then
        Set rs = New ADODB.Recordset
        rs.Open "DELETE FROM tb_status_kawin WHERE kd_kawin='" & Trim(txt_kode.Text) & "'", cn, 1, 2
        Kosong
        TampilData
    End If
End If
...
If txt_nama.Text = Empty Then
    MsgBox "Maaf, Anda belum mengisi Status Kawin", vbInformation, "Info"
    txt_nama.SetFocus
    Exit Sub
End If
...
If rs.EOF Then
    rs.AddNew
    rs!kd_kawin = Trim(txt_kode.Text)
    rs!status_kawin = Trim(txt_nama.Text)
    rs.Update
Else
    rs!status_kawin = Trim(txt_nama.Text)
    rs.Update
...
On Error Resume Next
  Dim rs As New ADODB.Recordset
    ...
    Set rs = New ADODB.Recordset
    rs.Open "SELECT * FROM tb_status_kawin WHERE kd_kawin='" & pID & "'", cn, 1, 2
    If rs.EOF Then
        GetList = False
    Else
    With rs
        txt_kode.Text = !kd_kawin
        txt_nama.Text = !status_kawin
...
Sub TampilData()
Set rs = New ADODB.Recordset
rs.Open "SELECT * FROM tb_status_kawin", cn, 1, 2
