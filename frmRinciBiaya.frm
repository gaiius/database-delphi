...
Begin VB.Form frmRinciBiaya 
   ...
   Begin Threed.SSPanel Panel 
      ...
      Begin Threed.SSFrame Frame1 
         ...
         Begin VB.ComboBox cbo_SPJPK 
            ...
            Width           =   1635
         End
         Begin MSComCtl2.DTPicker tgl_Rincian 
            Height          =   330
            Left            =   1800
            TabIndex        =   14
            Top             =   720
            ...
            CurrentDate     =   39995
         End
         Begin AdvancedTextBox.adTextBox txt_praktek 
            Height          =   285
            Left            =   1800
            TabIndex        =   2
            Top             =   2160
         ...
         Begin AdvancedTextBox.adTextBox txt_dokter 
            ...
            PropRegularExpression=   ""
         End
         Begin AdvancedTextBox.adTextBox txt_periksa 
            Height          =   285
            Left            =   1800
            TabIndex        =   4
            Top             =   2880
            ...
            PropRegularExpression=   ""
         End
         Begin AdvancedTextBox.adTextBox txt_obat 
            Height          =   285
            Left            =   1800
            TabIndex        =   5
            Top             =   3240
   ...
   End
End
Attribute VB_Name = "frmRinciBiaya"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
Public rs As ADODB.Recordset
Public rsSPJPK As ADODB.Recordset

Private Sub cbo_SPJPK_Click()
CariSPJPK
End Sub

Private Sub cbo_SPJPK_KeyPress(KeyAscii As Integer)
If KeyAscii = 13 Then
    SendKeys "{Tab}"
    CariSPJPK
End If
...
settombol True
cbo_SPJPK.Enabled = True
Kosong
End Sub

Private Sub cmdHapus_Click()
If cbo_SPJPK.Text = Empty Then
    MsgBox "Maaf, Nomor SPJPK tidak boleh kosong", vbInformation, "Info"
    Exit Sub
End If
...
If txt_praktek.Text = Empty Then
    MsgBox "Maaf, Praktek tidak boleh kosong", vbInformation, "Info"
    Exit Sub
End If
...
rs.Open "SELECT * FROM tb_rinci_biaya WHERE no_rincian='" & lbl_No.Caption & "'", cn, 1, 2
If rs.EOF Then
    MsgBox "Maaf, Tidak ada data yang dapat dihapus", vbInformation, "Info"
    Exit Sub
Else
    If MsgBox("Anda yakin akan menghapus data : " & lbl_No.Caption, vbYesNo, "Hapus Data") = vbYes Then
        Set rs = New ADODB.Recordset
        rs.Open "DELETE FROM tb_rinci_biaya WHERE no_rincian='" & lbl_No.Caption & "'", cn, 1, 2
        Kosong
        TampilData
        settombol True
    End If
...
If txt_dokter.Text = Empty Then
    ...
    Exit Sub
End If
If txt_periksa.Text = Empty Then
    MsgBox "Maaf, Biaya Periksa tidak boleh kosong", vbInformation, "Info"
    txt_periksa.SetFocus
    Exit Sub
End If
If txt_obat.Text = Empty Then
    MsgBox "Maaf, Biaya Obat tidak boleh kosong", vbInformation, "Info"
    txt_obat.SetFocus
    Exit Sub
End If
...
If rs.EOF Then
    With rs
        .AddNew
        !no_rincian = Trim(lbl_No.Caption)
        !tgl_Rincian = Format(tgl_Rincian.Value, "dd/MM/yyyy")
        !no_spjpk = Trim(cbo_SPJPK.Text)
        !praktek = Trim(txt_praktek.Text)
        !nm_dokter = Trim(txt_dokter.Text)
        !biaya_periksa = Abs(txt_periksa.Text)
        !biaya_obat = Abs(txt_obat.Text)
        .Update
    End With
Else
    If MsgBox("Nomor Rincian sudah ada, Apakah Anda akan mengubahnya?", vbYesNo, "Ubah Data") = vbYes Then
        With rs
            !praktek = Trim(txt_praktek.Text)
            !nm_dokter = Trim(txt_dokter.Text)
            !biaya_periksa = Abs(txt_periksa.Text)
...
tgl_Rincian.Value = Format(Now, "dd/MM/yyyy")
End Sub

Sub ItemCombo()
Set rsSPJPK = New ADODB.Recordset
rsSPJPK.Open "SELECT * FROM tb_spjpk ORDER BY no_spjpk", cn, 1, 2
If rsSPJPK.EOF Then
    cbo_SPJPK.Text = Empty
Else
    rsSPJPK.MoveFirst
    Do Until rsSPJPK.EOF
        cbo_SPJPK.AddItem rsSPJPK!no_spjpk
        rsSPJPK.MoveNext
    Loop
End If
...
Sub CariSPJPK()
Set rsSPJPK = New ADODB.Recordset
rsSPJPK.Open "SELECT * FROM view_SPJPK WHERE no_spjpk='" & Trim(cbo_SPJPK.Text) & "'", cn, 1, 2
If rsSPJPK.EOF Then
    lbl_Nama.Caption = Empty
    lbl_Divisi.Caption = Empty
    lbl_nama_pasien.Caption = Empty
    txt_praktek.Text = Empty
    txt_dokter.Text = Empty
Else
    With rsSPJPK
        lbl_Nama.Caption = !nik & " - " & !nm_karyawan
        lbl_Divisi.Caption = ValidNull(!divisi) & " - " & ValidNull(!jabatan)
        lbl_nama_pasien.Caption = ValidNull(!hub_kel) & " - " & ValidNull(!nama_pasien)
        txt_praktek.Text = ValidNull(!praktek)
        txt_dokter.Text = ValidNull(!dokter)
    End With
    
    Set rs = New ADODB.Recordset
    rs.Open "SELECT * FROM view_rincian_biaya WHERE no_rincian='" & Trim(cbo_SPJPK.Text) & "'", cn, 1, 2
    If rs.EOF Then
        tgl_Rincian.Value = Format(Now, "dd/MM/yyyy")
        txt_periksa.Text = 0
        txt_obat.Text = 0
        lbl_T_biaya.Caption = 0
        NoRincian
    Else
    With rs
        lbl_No.Caption = Trim(!no_rincian)
        tgl_Rincian.Value = Format(!tgl_Rincian, "dd/MM/yyyy")
'        txt_praktek.Text = ValidNull(!praktek)
'        txt_dokter.Text = ValidNull(!nm_dokter)
        txt_periksa.Text = Format(ValidNull(!biaya_periksa), "###,###,###")
        txt_obat.Text = Format(ValidNull(!biaya_obat), "###,###,###")
    End With
    lbl_T_biaya.Caption = Format(Val(Abs(txt_periksa.Text)) + Val(Abs(txt_obat.Text)), "###,###,###")
    End If
...
Set rs = New ADODB.Recordset
rs.Open "select * from tb_rinci_biaya order by no_rincian", cn, 1, adLockOptimistic

If Not rs.EOF Then
rs.MoveLast
NomorKwitansi = Val(Right(rs("no_rincian"), 6)) + 1
...
Sub TampilData()
Set rs = New ADODB.Recordset
rs.Open "SELECT no_rincian,tgl_rincian,no_spjpk,tgl_spjpk,nik,nm_karyawan FROM view_rincian_biaya", cn, 1, 2

Set Grid1.DataSource = rs
DoRefreshGrid Grid1, rs
With Grid1
    .Columns(0).Caption = "No.Rincian"
    .Columns(0).Width = 1500
    .Columns(1).Caption = "Tanggal"
    .Columns(1).Width = 1200
    .Columns(2).Caption = "No.SPJPK"
    .Columns(2).Width = 1500
    .Columns(3).Caption = "Tgl SPJPK"
    .Columns(3).Width = 1200
    .Columns(4).Caption = "NIK"
    .Columns(4).Width = 1500
    .Columns(5).Caption = "Nama Karyawan"
...
'On Error Resume Next
    ...
    Set rs = New ADODB.Recordset
    rs.Open "SELECT * FROM view_rincian_biaya WHERE no_rincian='" & pID & "'", cn, 1, 2
    If rs.EOF Then
        GetList = False
    Else
    With rs
        lbl_No.Caption = Trim(!no_rincian)
        tgl_Rincian.Value = Format(!tgl_Rincian, "dd/MM/yyyy")
        cbo_SPJPK.Text = ValidNull(!no_spjpk)
        txt_praktek.Text = ValidNull(!praktek)
        txt_dokter.Text = ValidNull(!nm_dokter)
        txt_periksa.Text = Format(ValidNull(!biaya_periksa), "###,###,###")
        txt_obat.Text = Format(ValidNull(!biaya_obat), "###,###,###")
...
Private Sub txt_obat_Change()
On Error Resume Next
lbl_T_biaya.Caption = Format(Val(Abs(txt_periksa.Text)) + Val(Abs(txt_obat.Text)), "###,###,###")
End Sub

Private Sub txt_obat_GotFocus()
On Error Resume Next
txt_obat.Text = Abs(txt_obat.Text)
End Sub

Private Sub txt_obat_LostFocus()
On Error Resume Next
txt_obat.Text = Format(txt_obat.Text, "###,###,###")
End Sub

Private Sub txt_periksa_Change()
On Error Resume Next
Dim Total As Double
Total = Val(Abs(txt_praktek.Text)) + Val(Abs(txt_obat.Text))
lbl_T_biaya.Caption = Format(Total, "###,###,###")
End Sub

Private Sub txt_periksa_GotFocus()
On Error Resume Next
txt_periksa.Text = Abs(txt_periksa.Text)
End Sub

Private Sub txt_periksa_LostFocus()
On Error Resume Next
txt_periksa.Text = Format(txt_periksa.Text, "###,###,###")
End Sub
